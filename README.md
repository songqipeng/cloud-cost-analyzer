...
```bash
# 克隆项目
git clone https://github.com/songqipeng/multi-cloud-cost-analyzer.git
...
# 添加定时任务（每天早上8点执行多云分析）
0 8 * * * cd /path/to/multi-cloud-cost-analyzer && python3 cloud_cost_analyzer.py multi-cloud >> cron.log 2>&1
```
...
5. **设置定时任务**（可选）
   ...
   # 每天早上8点自动运行
   crontab -e
   # 添加：0 8 * * * cd /path/to/multi-cloud-cost-analyzer && python3 cloud_cost_analyzer.py multi-cloud >> cron.log 2>&1
   ```
