...
class TencentDataProcessor:
        ...
        try:
            ...
            for item in summary_data:
                record = {
                    ...
                    'Region': 'Unknown',  # 腾讯云汇总数据通常没有区域信息
                    'Cost': float(item.get('real_total_cost', 0)),
                    'Currency': item.get('currency', 'CNY'),
                    'Provider': 'tencent',
                    'ProductCode': item.get('product_code', ''),
                    'TotalCost': float(item.get('total_cost', 0)),
                    'CashPayAmount': float(item.get('cash_pay_amount', 0)),
                    'VoucherPayAmount': float(item.get('voucher_pay_amount', 0)),
                    'IncentivePayAmount': float(item.get('incentive_pay_amount', 0))
                }
            ...
            if not all_records:
                logger.warning("没有找到符合条件的腾讯云费用记录")
                return pd.DataFrame()
            ...
            logger.info(f"解析腾讯云费用数据完成: {len(df)} 条记录")
            return df
            
        except Exception as e:
            logger.error(f"解析腾讯云费用数据失败: {e}")
            return pd.DataFrame()
    ...
    def analyze_costs_by_service(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"腾讯云服务费用分析完成: {len(service_stats)} 个服务")
            return service_stats
            
        except Exception as e:
            logger.error(f"腾讯云服务费用分析失败: {e}")
            return pd.DataFrame()
    ...
    def analyze_costs_by_region(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"腾讯云区域费用分析完成: {len(region_stats)} 个区域")
            return region_stats
            
        except Exception as e:
            logger.error(f"腾讯云区域费用分析失败: {e}")
            return pd.DataFrame()
